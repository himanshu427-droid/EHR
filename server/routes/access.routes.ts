...
const findPatientQuerySchema = z.object({
  query: z
    ...
    .optional(), // Make optional initially? Or require?
});
...
router.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const existing = await storage.getAccessByPatientAndEntity(
...
router.get(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const matchingPatients = await storage.findPatientsByNameOrUsername(
        query
      );
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      console.log(
        `Received access request from Doctor ${doctorId} for Patient ${patientId}`
      );
      ...
      if (existingAccess) {
        console.log(
          `Existing access record found with status: ${existingAccess.status}`
        );
        if (
          ...
          return res
            .status(400)
            .json({
              message: `Request status for this patient is already '${existingAccess.status}'`,
            });
        ...
        console.log(
          "No existing record found. Creating new pending request..."
        );
        const accessData = {
          patientId,
          entityId: doctorId,
...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const accessGrant = await storage.getAccessByPatientAndEntity(
        patientId,
        requestingEntityId
      );
      ...
      if (!accessGrant || accessGrant.status != "active") {
        console.log(
          `Access denied for ${requestingEntityId} to view records of ${patientId}. Status: ${accessGrant?.status}`
        );
        return res
          .status(403)
          .json("Access denied: You don't have this permission!");
      }
      ...
      console.error(
        `Error fetching records for patient ${req.params.patientId}:`,
        error
      );
...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const accessRecords = await storage.getAccessByPatient(req.user!.userId);
      ...
      const entityDataPromises = pendingRecords.map(async (access) => {
        ...
        if (!entity) {
          console.warn(
            `Entity user not found for access record ID: ${access.id}, Entity ID: ${access.entityId}`
          );
          return null;
        }
        return {
          ...
          entity: {
            ...
            role: entity.role as UserRoleType,
          },
      ...
      console.log(
        `Returning ${pendingRequests.length} detailed pending requests.`
      );
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      console.log(
        `Checking ownership: Record patientId=${
          access.patientId
        }, Requesting userId=${req.user!.userId}`
      );
      if (access.patientId !== req.user!.userId) {
        console.log("Ownership check failed");
        return res
          .status(403)
          .json({ message: "You cannot approve this request" });
      }
